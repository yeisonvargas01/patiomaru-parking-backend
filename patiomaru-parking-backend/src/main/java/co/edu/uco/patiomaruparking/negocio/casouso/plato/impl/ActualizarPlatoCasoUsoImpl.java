...
public final class ActualizarPlatoCasoUsoImpl implements ActualizarPlatoCasoUso {
	...
	public void ejecutar(final PlatoDominio datos) {
		...
		// 2. Debe existir el plato que se desea actualizar.
		var platoActualEntidad = daoFactory.obtenerPlatoDAO().consultarPorId(codigoPlato);

		if (UtilObjeto.esNulo(platoActualEntidad)) {
			throw new RuntimeException("No existe un plato registrado con el código indicado.");
		}

		// 3. No debe existir otro plato con el mismo nombre dentro de la misma categoría.
		validarNoExisteOtroPlatoConMismoNombreYCategoria(datos);

		// 4. Actualizar información del plato.
	...
	private void validarNoExisteOtroPlatoConMismoNombreYCategoria(final PlatoDominio datos) {
		...
		for (PlatoEntidad plato : resultados) {
			if (!UtilTexto.sonIgualesIgnorandoMayusculas(
					plato.getCodigoPlato(), datos.getCodigoPlato())) {
	...
	private void actualizar(final PlatoDominio plato) {
		...
		daoFactory.obtenerPlatoDAO().actualizar(platoEntidad);
	}
}
